...
This is **eleven data figures** plus one schematic (LCSS Fig. 1).
**v0.7.1 update:** LCSS Fig. 1 is now also a programmatic figure — a
generic ABM-schematic generator (`viz.model_schematic.plot_model_schematic`)
that takes cell-type names + interaction rules and renders an SVG of
coloured circles + typed arrows. See [decision log: LCSS-1 in scope](../docs/development/decisions/2026-05-14-lcss-1-schematic-in-scope.md).
The original "ships as a static SVG asset" plan is retired.
...
| LCSS-1 | LCSS | **(v0.7.1)** Programmatic ABM model schematic: coloured-circle nodes + typed arrows from a user-supplied cell-type list + interaction rules | — | medium | **no reference** — generated by `viz.model_schematic.plot_model_schematic` |

LCSS-1 was originally listed as out-of-scope (static SVG asset). The
v0.7.1 ship promoted it to a generic programmatic schematic generator
per the [decision log](../docs/development/decisions/2026-05-14-lcss-1-schematic-in-scope.md).
The function works for any ABM, not just the LCSS paper's model.
...
1. ~~**LCSS Figure 1 schematic**: shipping as SVG asset, not Python
   function.~~ **Retired 2026-05-14.** Promoted to a programmatic
   schematic generator (`viz.model_schematic.plot_model_schematic`)
   that works for any user-supplied ABM. Shipped in v0.7.1.
